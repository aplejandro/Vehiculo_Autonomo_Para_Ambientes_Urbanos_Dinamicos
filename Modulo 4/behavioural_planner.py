...
FOLLOW_LANE = 0
DECELERATE_TO_STOP_SS = 1
STAY_STOPPED_SS = 2
DECELERATE_TO_STOP_TL = 3
STAY_STOPPED_TL = 4
...
class BehaviouralPlanner:
    def __init__(self, lookahead, stopsign_fences, trafficlight_fences, lead_vehicle_lookahead):
        self._lookahead                     = lookahead
        self._stopsign_fences               = stopsign_fences
        self._trafficlight_fences           = trafficlight_fences
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed, tl_state):
        """Handles state transitions and computes the goal state.
        ...
        if self._state == FOLLOW_LANE:
            ...
            if stop_sign_found:
                ...
                self._state = DECELERATE_TO_STOP_SS
            # ------------------------------------------------------------------
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            goal_index, traffic_light_found = self.check_for_traffic_lights(waypoints, closest_index, goal_index)
            self._goal_index = goal_index
            self._goal_state = waypoints[self._goal_index]
            # ------------------------------------------------------------------
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            if traffic_light_found and tl_state == 2:
                self._goal_state[2] = 0
                self._state = DECELERATE_TO_STOP_TL
            # ------------------------------------------------------------------
            print("traffic_light_found = %d" % traffic_light_found)
            pass
        ...
        # state.
        elif self._state == DECELERATE_TO_STOP_SS:
        ...
        # stop, and compare to STOP_THRESHOLD.  If so, transition to the next
        # state.
        elif self._state == DECELERATE_TO_STOP_TL:
            print("Decelerating to stop")
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            if closed_loop_speed > STOP_THRESHOLD:
        ...
        # the stop sign and transition to the next state.
        elif self._state == STAY_STOPPED_SS:
            ...
            # You should use the get_closest_index(), get_goal_index(), and
            # check_for_stop_signs() helper functions.
            if self._stop_count == STOP_COUNTS_SS:
        ...
        # the stop sign and transition to the next state.
            ...
            if tl_state == 0:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                traffic_light_found = self.check_for_traffic_lights(waypoints, closest_index, goal_index)[1]
                self._goal_index = goal_index
                self._goal_state = waypoints[self._goal_index]
                # --------------------------------------------------------------
    ...
    def check_for_traffic_lights(self, waypoints, closest_index, goal_index):
        ...
        for i in range(closest_index, goal_index):
            # Check to see if path segment crosses any of the stop lines.
            intersect_flag = False
            for trafficlight_fence in self._trafficlight_fences:
                wp_1   = np.array(waypoints[i][0:2])
                wp_2   = np.array(waypoints[i+1][0:2])
                s_1    = np.array(trafficlight_fence[0:2])
                s_2    = np.array(trafficlight_fence[2:4])
                ...
                # If there is an intersection with a traff light, update
                # the goal state to stop before the goal line.
                if intersect_flag:
                    goal_index = i
                    return goal_index, True
