...
WAYPOINTS_FILENAME = 'course4_waypoints_new.txt'  # waypoint file to load
...
C4_TRAFFIC_LIGHT_FILE        = 'traffic_light_params.txt'
C4_TRAFFIC_LIGHT_FENCELENGTH = 5        # m
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        #############################################
        # Load stop sign, traffic light and parked vehicle parameters
        ...
        trafficlight_fences = []     # [x0, y0, x1, y1]
        with open(C4_TRAFFIC_LIGHT_FILE, 'r') as trafficlight_file:
            next(trafficlight_file)  # skip header
            trafficlight_reader = csv.reader(trafficlight_file,
                                         delimiter=',',
                                         quoting=csv.QUOTE_NONNUMERIC)
            trafficlight_data = list(trafficlight_reader)
            # convert to rad
            for i in range(len(trafficlight_data)):
                trafficlight_data[i][3] = trafficlight_data[i][3] * np.pi / 180.0

        # obtain traffic light fence points for LP
        for i in range(len(trafficlight_data)):
            x = trafficlight_data[i][0]
            y = trafficlight_data[i][1]
            z = trafficlight_data[i][2]
            yaw = trafficlight_data[i][3] + np.pi / 2.0  # add 90 degrees for fence
            spos = np.array([
                    [0, 0                       ],
                    [0, C4_TRAFFIC_LIGHT_FENCELENGTH]])
            rotyaw = np.array([
                    [np.cos(yaw), np.sin(yaw)],
                    [-np.sin(yaw), np.cos(yaw)]])
            spos_shift = np.array([
                    ...
                    [y, y]])
            spos = np.add(np.matmul(rotyaw, spos), spos_shift)
            trafficlight_fences.append([spos[0,0], spos[1,0], spos[0,1], spos[1,1]])
        ...
        trajectory_fig.add_graph("trafficlight", window_size=1,
                                 x0=[trafficlight_fences[0][0]], y0=[trafficlight_fences[0][1]],
                                 marker="d", color="y",
                                 markertext="Traffic Light", marker_text_offset=1)

        # Add stop sign "stop line"
        trajectory_fig.add_graph("trafficlight_fence", window_size=1,
                                 x0=[trafficlight_fences[0][0], trafficlight_fences[0][2]],
                                 y0=[trafficlight_fences[0][1], trafficlight_fences[0][3]],
                                 color="r")
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            traffic_lights = []
            #f = open ("traffic_lights.txt", "a")
            #i = 1
            for agent_tl in measurement_data.non_player_agents:
                agent_id = agent_tl.id
                if agent_tl.HasField('traffic_light'):
                    traffic_lights.append(agent_tl)
                    break
                    #print("Traffic Light State: " + str(agent_tl.traffic_light.state))
                    #print("Traffic Light Transform: " + str(agent_tl.traffic_light.transform))
                    #f.write("TL: " + str(i) + "\n")
                    #f.write(str(agent_tl.traffic_light.transform))
                    #i += 1
            #print(str(len(traffic_lights)) + " Traffic Lights Found")
            #f.close()
            tl = traffic_lights[0]
            tl_state = tl.traffic_light.state
            #f = open("tl_state.txt","a")
            #print("TL State: " + str(tl.traffic_light.state))
            #f.write("TL State: " + str(tl.traffic_light.state) + "\n")
            #f.close()
            ...
            if frame % LP_FREQUENCY_DIVISOR == 0:
                ...
                bp.transition_state(waypoints, ego_state, current_speed, tl_state)
                ...
                decelerate_to_stop = bp._state == behavioural_planner.DECELERATE_TO_STOP_SS
