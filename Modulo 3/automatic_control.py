...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            spawn_point = spawn_points[8] if spawn_points else carla.Transform()
            print(str(len(spawn_points)) + "Spawn_points")
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
        # Set up the sensors.
...
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        self._camera_transforms = [
            ...
            carla.Transform(carla.Location(x=-5.5,y=0, z=2.8),carla.Rotation(pitch=-15.0))]
    ...
    def set_sensor_frame(self, index, notify=True):
        index = index % len(self.sensors)
        needs_respawn = True if self.index is None \
            else self.sensors[index][0] != self.sensors[self.index][0]
        if needs_respawn:
            ...
            # circular reference.
            weak_self = weakref.ref(self)
            self.sensor.listen(lambda image: CameraManager._parse_image_frame(weak_self, image))
        if notify:
            self.hud.notification(self.sensors[index][2])
        self.index = index
    ...
    @staticmethod
    def _parse_image_frame(weak_self, image):
        self = weak_self()
        if not self:
            return
        ...
        if self.recording:
            image.save_to_disk('output_map_2/frame/%08d' % image.frame_number)
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
        if self.recording:
            image.save_to_disk('output_map_2/rgb/%08d' % image.frame_number)
