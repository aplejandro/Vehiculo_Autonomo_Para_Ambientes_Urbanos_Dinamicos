...
def draw_lines(img, lines, color=[225, 255, 255], thickness=15):
    """
    ...
    ysize = img.shape[0]
    xsize = img.shape[1]
    y_top = int(ysize*0.6) # need y coordinates of the top and bottom of left and right lane
...
def detection_pipeline(image, kernel_size=9, canny_low=30, canny_high=90, rho=1, theta=np.pi/180,
                       threshold=20, min_line_len=40, max_line_gap=25):
...
test_images = os.listdir("output_map_3/rgb_3/")
im = test_images[0]
print(im)
path = 'output_map_3/rgb_3/'
img = cv2.imread('output_map_3/rgb_3/' + im)
