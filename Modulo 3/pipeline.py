...
def find_line_raw(warped, nwindows=9, return_img=False, plot_boxes=False, plot_line=False):
    ...
    max_l = np.argmax(histogram[:midpoint])
    max_r = np.argmax(histogram[midpoint:])
    ...
    #print("Left non zeros: " + str(len(leftx)))
    rightx = nonzerox[right_lane_inds]
    righty = nonzeroy[right_lane_inds]
    #print("Right non zeros: " + str(len(rightx)))
    if len(leftx) < 1000:
        left_fit = np.array([0.00011597548283811136, 0.014985237485758938, 66.06267955847044])
    else:
        # Fit a second order polynomial to each
        try:
            left_fit = np.polyfit(lefty, leftx, 2)
            #print('Left_fit' + ' ' + str(left_fit[0]) + ' ' + str(left_fit[1]) + ' ' + str(left_fit[2]))
        except:
            try:
                ...
                left_fit = np.array([0.00011597548283811136, 0.014985237485758938, 66.06267955847044])
    if len(rightx) < 1000:
        right_fit = np.array([-0.00019947343592381677, 0.10059062168929517, 1254.7871230468343])
    else:
        try:
            right_fit = np.polyfit(righty, rightx, 2)
            #print('Right_fit' + ' ' + str(right_fit[0]) + ' ' + str(right_fit[1]) + ' ' + str(right_fit[2]))
        except:
            try:
    ...
    if return_img:
        ...
        if plot_line:
            ...
            try:
                ...
                print('Error : The function failed to fit a line!')
